...
import {
  ...
  CheckUserResponse,
  CreateFundResponse,
} from '../dtos/prosper.dto';
import { decryptPrivateKey } from 'src/common/utils';

@Injectable()
export class ProsperCommonService {
  private readonly logger = new Logger(ProsperCommonService.name);
  ...
  private async checkUser(prosperId: string): Promise<CheckUserResponse> {
    ...
    try {
      ...
      return {
        ...
        balanceARSA: balances.balanceARSA,
    ...
    try {
      ...
      try {
        ...
        if (initialAmount && Number(initialAmount) > 0) {
          ...
          // const sourcePrivateKey = decryptPrivateKey(issuer.secretKey);
  ...
  public async deposit(payload: DepositDto): Promise<Partial<Transacciones>> {
    ...
    try {
      ...
      const tx = await this.stellarService.makeTransaction({
        asset: payload.asset,
        sourcePublicKey: teasureKey.publicKey,
        sourcePrivateKey: teasureKey.secretKey,
  ...
  public async retire(payload: DepositDto): Promise<Partial<Transacciones>> {
    ...
    try {
      ...
      if (payload.asset === 'ARSa') {
        currentBalance = balance.balanceARSA;
      } else if (payload.asset === 'USDC') {
        currentBalance = balance.balanceUSDC;
      }
      ...
      const sourcePrivateKey = decryptPrivateKey(user.secret);
      const tx = await this.stellarService.makeTransaction({
        asset: payload.asset,
        sourcePublicKey: user.address,
        sourcePrivateKey,
  ...
  public async transfer(payload: TransferDto): Promise<Partial<Transacciones>> {
    ...
    try {
      const { fromUserId, toUserId, asset, amount, prosperTxId, metadata } =
        payload;
      if (!prosperTxId || prosperTxId.trim() === '') {
        throw new BadRequestException('prosperTxId es requerido');
      }
      ...
      let currentBalance = '0';
      if (asset === 'ARSa') {
        currentBalance = fromUser.balanceARSA;
      } else if (asset === 'USDC') {
        currentBalance = fromUser.balanceUSDC;
      }
      ...
      const sourcePrivateKey = decryptPrivateKey(fromUser.secret);
      const tx = await this.stellarService.makeTransaction({
        asset,
        sourcePublicKey: fromUser.address,
        sourcePrivateKey,
  ...
  public async getBalanceByWallet(address: string): Promise<GetBalanceResponse> {
    this.logger.debug('getBalance by Wallet called');
    try {
      const user = await this.checkUserAddress(address);
      const { secret, ...balanceInfo } = user;
      const _secret = secret;
      return balanceInfo;
    } catch (error) {
      this.logger.error(
        `Error en getBalance by Wallet: `,
        error instanceof Error ? error.message : String(error),
      );
      throw new BadRequestException(
        'Error obteniendo balance by Wallet',
        error instanceof Error ? error.message : String(error),
      );
    }
  }
